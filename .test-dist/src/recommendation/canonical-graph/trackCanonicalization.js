...
exports.recommendationTrackCanonicalizationService = void 0;
...
function getTrackMetadataQuality(track) {
    ...
    const coverQualityScore = (0, coverUrl_1.getCoverUrlQualityScore)(track.coverUrl);
...
function prepareTrack(track) {
    const normalized = normalization_1.recommendationNormalizationService.normalizeTrackForCanonicalization(track);
    const prepared = {
        ...track,
        normalizedTitle: track.normalizedTitle ?? normalization_1.recommendationNormalizationService.normalizeTrackTitle(track.title),
        normalizedArtistName: track.normalizedArtistName ??
            normalization_1.recommendationNormalizationService.normalizeArtistName(track.artist),
        ...
        sourcePriority: track.sourcePriority ?? (0, sourceMetadata_1.getSourcePriority)(track.providerId),
        sourceTrustScore: track.sourceTrustScore ?? (0, sourceMetadata_1.getSourceTrustScore)(track.providerId),
...
function selectCoverVariant(variants) {
    return selectTrackByField(variants, (variant) => variant.coverUrl, undefined, (variant) => (0, coverUrl_1.getCoverUrlQualityScore)(variant.coverUrl));
}
...
function buildCanonicalId(variants, preferredVariant, targetDuration) {
    ...
    const key = [
        ...
        normalization_1.recommendationNormalizationService.getDurationBucket(targetDuration),
    ].join("|");
...
function buildClusterDebugInfo(variants, blockingKeys, pairScoring, remaps, includeDebugInfo) {
    ...
    return {
        ...
        mergeBlockers: dedupeStrings(relevantPairs.flatMap((pair) => pair.blockers)),
        clusterReasons: dedupeStrings(relevantPairs.flatMap((pair) => pair.reasons)),
...
function buildCanonicalLyricsCacheKey(canonicalTrack, config) {
    const durationBucket = normalization_1.recommendationNormalizationService.getDurationBucket(canonicalTrack.targetDuration ?? 0, config.lyricsDurationBucketMs / 1000);
    return [
        normalization_1.recommendationNormalizationService.normalizeTrackTitleCore(canonicalTrack.title),
        normalization_1.recommendationNormalizationService.normalizeArtistCore(canonicalTrack.artist),
        durationBucket,
    ].join("|");
}
exports.recommendationTrackCanonicalizationService = {
    canonicalizationVersion: CANONICALIZATION_VERSION,
    buildCanonicalizationResult({ searchSetId, tracks, lyricsByTrackId = {}, previousResult, canonicalizationRevision, config, includeDebugInfo = true, }) {
        const activeConfig = config ?? defaultRecommendationConfig_1.defaultRecommendationConfig.canonicalization;
        ...
        const canonicalTracks = mergedClusterVariants
            ...
            return {
                ...
                coverUrl: (0, coverUrl_1.pickBestCoverUrl)(coverVariant.coverUrl, variants.find((variant) => !(0, coverUrl_1.isPlaceholderCoverUrl)(variant.coverUrl))?.coverUrl) || null,
