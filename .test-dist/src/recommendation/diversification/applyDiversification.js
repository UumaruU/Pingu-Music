...
exports.applyDiversification = applyDiversification;
function clamp(value, min, max) {
    return Math.min(max, Math.max(min, value));
}
// Pure domain logic: diversification is applied after scoring and before final selection.
function applyDiversification(params) {
    const recentArtistCounts = params.context.recentArtistIds.reduce((accumulator, artistId) => {
        accumulator[artistId] = (accumulator[artistId] ?? 0) + 1;
        return accumulator;
    }, {});
    const recentReleaseCounts = params.context.recentTrackIds.reduce((accumulator, trackId) => {
    ...
    const recentTagCounts = params.context.recentTrackIds.reduce((accumulator, trackId) => {
        const track = params.snapshot.tracksById[trackId];
    ...
    return params.candidates
        ...
        const cappedPenalty = clamp(extraPenalty, 0, Math.max(0, candidate.scoreBreakdown.finalScore * params.config.diversification.maxPenaltyShare));
