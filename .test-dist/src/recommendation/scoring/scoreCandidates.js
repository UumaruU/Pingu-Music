...
function normalizeDurationScore(leftDurationMs, rightDurationMs) {
...
function buildRecentArtistCounts(context) {
    return context.recentArtistIds.reduce((accumulator, artistId) => {
        accumulator[artistId] = (accumulator[artistId] ?? 0) + 1;
        return accumulator;
    }, {});
}
function buildRecentTrackCounts(context) {
    return context.recentTrackIds.reduce((accumulator, trackId) => {
        accumulator[trackId] = (accumulator[trackId] ?? 0) + 1;
        return accumulator;
    }, {});
}
function buildRecentReleaseCounts(snapshot, context) {
    return context.recentTrackIds.reduce((accumulator, trackId) => {
...
function buildRecentTagCounts(snapshot, context) {
    return context.recentTrackIds.reduce((accumulator, trackId) => {
        const track = snapshot.tracksById[trackId];
        if (!track) {
            return accumulator;
...
function scoreCandidates(params) {
    ...
    return candidates
        ...
        const sameArtistScore = currentTrack?.primaryCanonicalArtistId && track.primaryCanonicalArtistId === currentTrack.primaryCanonicalArtistId
            ? 1
            : currentTrack?.featuringCanonicalArtistIds.some((artistId) => track.canonicalArtistIds.includes(artistId))
                ? 0.5
                : 0;
        const collaboratorScore = currentTrack?.primaryCanonicalArtistId && track.primaryCanonicalArtistId
            ? (snapshot.artistRelations[currentTrack.primaryCanonicalArtistId] ?? []).find((edge) => edge.rightId === track.primaryCanonicalArtistId)?.weight ?? 0
            : 0;
        const relatedArtistScore = currentTrack?.primaryCanonicalArtistId && track.primaryCanonicalArtistId
            ? (snapshot.relatedArtists[currentTrack.primaryCanonicalArtistId] ?? []).find((edge) => edge.rightId === track.primaryCanonicalArtistId)?.weight ?? 0
            : 0;
        const tagOverlapScore = weightedJaccard(currentTrack?.tagWeights ?? context.recentTagCloud, track.tagWeights);
        const sessionFitScore = weightedJaccard(sessionTagWeights, track.tagWeights);
        const releaseProximityScore = currentTrack?.canonicalReleaseId && track.canonicalReleaseId === currentTrack.canonicalReleaseId
            ? 1
            : currentTrack?.canonicalReleaseId &&
                ...
                : currentTrack?.year && track.year && Math.abs(currentTrack.year - track.year) <= 2
                    ? 0.25
                    : 0;
        const tasteAffinityScore = (profiles.entity.trackAffinities[track.canonicalTrackId]?.value ?? 0) * 0.2 +
            (track.primaryCanonicalArtistId
                ? (profiles.entity.artistAffinities[track.primaryCanonicalArtistId]?.value ?? 0) * 0.12
                : 0) +
            track.tagIds.reduce((accumulator, tagId) => accumulator + (profiles.entity.tagAffinities[tagId]?.value ?? 0) * 0.05, 0) +
        ...
        const flavorFitScore = context.currentFlavor && track.titleFlavor.includes(context.currentFlavor)
            ? 1
            : profiles.session.dominantFlavor && track.titleFlavor.includes(profiles.session.dominantFlavor)
                ...
                : track.titleFlavor.includes("original")
                    ? 0.2
                    : 0;
        const qualityScore = track.quality.clusterConfidence * 0.5 +
        ...
        const repetitionPenalty = Math.min(1, (track.primaryCanonicalArtistId ? (recentArtistCounts[track.primaryCanonicalArtistId] ?? 0) : 0) /
        ...
        const explicitMismatchPenalty = profiles.entity.dislikedTrackIds.includes(track.canonicalTrackId) ||
        ...
        const weightedPositive = sameArtistScore * config.scoringWeights.sameArtist +
        ...
        const weightedPenalty = repetitionPenalty * config.scoringWeights.repetitionPenalty +
