...
function pushCandidate(bag, canonicalTrackId, channel, weight, evidence) {
...
function topAffinityKeys(entries, limit) {
...
function generateCandidates(params) {
    ...
    const currentTrack = (context.currentCanonicalTrackId && snapshot.tracksById[context.currentCanonicalTrackId]) ||
    ...
    if (currentTrack?.primaryCanonicalArtistId) {
        ...
        sameArtistTracks
            ...
            .forEach((trackId) => pushCandidate(candidates, trackId, "sameArtist", 1.2, {
    ...
    if (currentTrack?.canonicalReleaseId) {
        (snapshot.releaseToTracks[currentTrack.canonicalReleaseId] ?? [])
            ...
            .forEach((trackId) => pushCandidate(candidates, trackId, "releaseEraProximity", 1, {
    ...
    topAffinityKeys(profiles.entity.artistAffinities, 10).forEach((artistId) => {
        (snapshot.artistToTracks[artistId] ?? []).forEach((trackId) => {
            pushCandidate(candidates, trackId, "userAffinityRetrieval", profiles.entity.artistAffinities[artistId]?.value ?? 0, { artistId });
        });
    });
    topAffinityKeys(profiles.entity.tagAffinities, 12).forEach((tagId) => {
        (snapshot.tagToTracks[tagId] ?? []).forEach((trackId) => {
            pushCandidate(candidates, trackId, "userAffinityRetrieval", profiles.entity.tagAffinities[tagId]?.value ?? 0, { tagId });
        });
    });
    topAffinityKeys(profiles.entity.trackAffinities, 12).forEach((trackId) => {
        if (snapshot.tracksById[trackId]) {
            pushCandidate(candidates, trackId, "userAffinityRetrieval", profiles.entity.trackAffinities[trackId]?.value ?? 0, { affinityTrackId: trackId });
        }
    });
