...
import { createRecommendationEngine } from "../src/recommendation";
...
function createMemoryCacheStore(): RecommendationCacheStore {
...
function createContext(): RecommendationContext {
  return {
    mode: "autoplay",
...
function createEngine(snapshot: RecommendationCatalogSnapshot) {
  ...
  return createRecommendationEngine(dependencies, defaultRecommendationConfig);
}

test("returns deterministic next-track recommendations with preferredVariantId", async () => {
  ...
  const ranking = await engineForRanking.getRecommendedTracks(
    { mode: "autoplay", canonicalTrackId: "track:current" },
    context,
  );

  assert.ok(first);
  assert.equal(first?.canonicalTrackId, "track:related");
  assert.equal(first?.preferredVariantId, "variant:related");
  assert.deepEqual(first, second);
  assert.deepEqual(
    ranking.map((item) => item.canonicalTrackId),
    ["track:related", "track:same-artist"],
  );
});

test("deduplicates tracks found through multiple channels and filters broken candidates", async () => {
  const snapshot = createSnapshot();
  const engine = createEngine(snapshot);
  const ranking = await engine.getRecommendedTracks(
    { mode: "autoplay", canonicalTrackId: "track:current" },
    createContext(),
  );

  assert.equal(
    ranking.filter((item) => item.canonicalTrackId === "track:same-artist").length,
    1,
  );
  assert.equal(ranking.some((item) => item.canonicalTrackId === "track:broken"), false);
});

test("favorite reinforcement and early skip penalties affect ranking", async () => {
  const snapshot = createSnapshot();
  const engine = createEngine(snapshot);
  const context = createContext();

  const before = await engine.getRecommendedTracks(
    { mode: "autoplay", canonicalTrackId: "track:current" },
    context,
  );
  assert.equal(before[0]?.canonicalTrackId, "track:related");
  ...
  const afterFavorite = await engine.getRecommendedTracks(
    { mode: "autoplay", canonicalTrackId: "track:current" },
    context,
  );
  assert.equal(afterFavorite[0]?.canonicalTrackId, "track:same-artist");
  ...
  const afterSkip = await engine.getRecommendedTracks(
    { mode: "autoplay", canonicalTrackId: "track:current" },
    context,
  );
  assert.equal(afterSkip[0]?.canonicalTrackId, "track:related");
});
